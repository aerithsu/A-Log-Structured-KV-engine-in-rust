...
impl Engine {
	...
	pub fn fold<F>(&self, f: F) -> Result<()>
		where Self: Sized, F: Fn(Bytes, Bytes) -> bool
	{
...
impl Iterator<'_> {
	...
	pub fn next(&self) -> Option<(Bytes, Bytes)> {
		...
		if let Some(item) = index_iter.next() {
			let value = self.engine.get_value_by_position(item.1.clone()).expect("failed to get value from data file");
...
mod tests {
	...
	fn test_iterator_seek() {
		...
		engine.put(Bytes::from("aacc"), util::rand_kv::get_test_value(10))
		...
		engine.put(Bytes::from("abbb"), util::rand_kv::get_test_value(10))
			.expect("Engine failed to put");
		engine.put(Bytes::from("accc"), util::rand_kv::get_test_value(20))
			.expect("Engine failed to put");
		engine.put(Bytes::from("abbb"), util::rand_kv::get_test_value(30))
			.expect("Engine failed to put");
		let iter = engine.iter(&IteratorOptions::default());
		iter.seek("a".as_bytes().to_vec());
		while let Some((key, value)) = iter.next() {
		...
		//删除临时文件
		fs::remove_dir_all(PathBuf::from("/tmp/bitcask-rs-iter-seek")).expect("failed to remove the dir");
	}
	...
	fn test_iterator_next() {
		...
		let engine = Engine::open(opts).expect("failed to open engine");
		engine.put(Bytes::from("accc"), util::rand_kv::get_test_value(20)).expect("failed to put");
